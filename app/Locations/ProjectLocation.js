...
class ProjectLocation extends React.Component {
  ...
  chooseLocation = () => {
    ...
    console.log("itemchoose", item);
    {
      isMount
        ? fetch(
            urlApi + "c_location/chooseLocation/IFCAMOBILE/" + item.location,
  ...
  selectedItem = (item) => {
    ...
    let status = item.project_status;
    console.log("status aktif", status);
    // alert(val);
    // onPress={()=>Actions.propertydetail({items:item})}
    if (item) {
      Actions.propertydetail({ items: item, status_aktif: status });
      // setTimeout(() => {
  ...
  render() {
    ...
    return (
      <Container style={Style.bgMain}>
        ...
        >
          <View
            style={[Style.actionBarLeft, { paddingTop: 35, paddingLeft: 20 }]}
          >
            <Button
              transparent
              style={Style.actionBarBtn}
          ...
          {this.state.chooselocation == 0 ? (
            <ActivityIndicator color="#000" style={{ paddingTop: 10 }} />
          ) : (
            <List>
              <FlatList
